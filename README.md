...
```bash
lftp -e "set ssl:verify-certificate no; open -u test-user,change-it 127.0.0.1:2121; \ 
  put -O . /dev/null -o test-empty1.txt; \
  ls -l; \
  rm test-empty1.txt; \
  bye"
```
...
```bash
lftp -e "set ssl:ca-file /var/vcap/jobs/proftpd/config/mod_tls/ca.crt; open -u test-user,change-it q-s0.proftpd.default.proftpd-rnd.bosh:2121; \
  put -O . /dev/null -o test-empty2.txt; \
  ls -l; \
  rm test-empty2.txt; \
  bye"
```
...
lftp -e "set ssl:verify-certificate no; open -u test-user,change-it 127.0.0.1:2121; \ 
  put -O . /dev/null -o test-empty3.txt; \
  ls -l; \
  bye"
  
lftp -e "set ssl:verify-certificate no; open -u test-user-ro,change-it 127.0.0.1:2121; \ 
  ls -l; \
  rm test-empty3.txt; \
  bye"

lftp -e "set ssl:verify-certificate no; open -u test-user-ro,change-it 127.0.0.1:2121; \ 
  put -O . /dev/null -o test-empty4.txt; \
  bye"
...
```bash
sftp -P 2222 test-user@127.0.0.1
```

```bash
lftp -e "set sftp:auto-confirm yes" sftp://test-user@127.0.0.1:2222 
```
